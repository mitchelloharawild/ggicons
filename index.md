...
draws a value as a grid of repeated icons, some fraction of them filled
in and the rest left faded, the way an isotype chart uses “each icon = n
units” instead of a bar’s length. Leaving `y` out of
[`aes()`](https://ggplot2.tidyverse.org/reference/aes.html) grows the
grid upward like a column chart, with `value` mapped to how many icons
are filled and `icon`/`colour` mapped to category:
...
![](reference/figures/README-geom-pictogram-1.png)
...
spells this ratio out in the legend automatically.
