...
`geom_icon()` is a
[`ggplot2::geom_point()`](https://ggplot2.tidyverse.org/reference/geom_point.html)-shaped
geom that draws an icon per row at `(x, y)`, instead of a point. Icons
come from the [icons](https://github.com/mitchelloharawild/icons)
package: single-fill SVG vector shapes, parsed once per unique icon and
cached, then recoloured/rescaled/rotated per row at draw time.
...
- size.unit:
  ...
  The unit in which the `size` aesthetic is interpreted, one of `"mm"`
  (default), `"pt"`, `"cm"`, `"in"`, or another
...
- **`icon`**: the icon to draw, as an icon vector from the
  ...
  result. Character ids are not resolved; map an actual icon vector.
...
| • | [`size`](https://ggplot2.tidyverse.org/reference/aes_linetype_size_shape.html) | → `6` |
